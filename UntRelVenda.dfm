inherited FrmRelVenda: TFrmRelVenda
  Caption = 'FrmRelVenda'
  ExplicitWidth = 651
  ExplicitHeight = 338
  PixelsPerInch = 96
  TextHeight = 13
  inherited PTopo: TPanel
    inherited LCad: TLabel
      Width = 639
      Height = 30
    end
  end
  inherited Relatorio: TfrxReport
    ReportOptions.LastChange = 43611.674299872700000000
    Datasets = <
      item
        DataSet = DBITEM
        DataSetName = 'ITEM'
      end
      item
        DataSet = DBDS
        DataSetName = 'Principal'
      end>
    Variables = <>
    Style = <>
    inherited Page1: TfrxReportPage
      inherited PageFooter1: TfrxPageFooter
        Top = 487.559370000000000000
        Width = 718.110700000000000000
        inherited Memo1: TfrxMemoView
          Formats = <
            item
            end
            item
            end>
        end
        inherited Line1: TfrxLineView
          Top = 1.779530000000022000
          Width = 718.110700000000000000
        end
      end
      inherited PageHeader1: TfrxPageHeader
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        inherited Picture1: TfrxPictureView
          Top = 7.559059999999999000
          Picture.Data = {
            
            38B22E7D0000000049454E44AE426082}
        end
        inherited Memo4: TfrxMemoView
          Left = 255.118275000000000000
          Top = 7.559059999999999000
        end
        inherited Memo5: TfrxMemoView
          Left = 308.031694999999900000
          Top = 64.252010000000000000
        end
        inherited Line2: TfrxLineView
          Left = -0.000000000000024203
          Top = 98.267780000000000000
        end
        inherited Memo2: TfrxMemoView
          Left = 582.047620000000000000
          Top = 71.811070000000000000
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 147.401670000000000000
        Top = 177.637910000000000000
        Width = 718.110700000000000000
        DataSet = DBDS
        DataSetName = 'Principal'
        RowCount = 0
        object Memo11: TfrxMemoView
          Left = 7.559060000000000000
          Top = 7.559059999999988000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'C'#243'digo')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 147.401670000000000000
          Top = 7.559059999999988000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Data')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 287.244280000000000000
          Top = 7.559059999999988000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Baixa')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Left = 7.559060000000000000
          Top = 33.015770000000010000
          Width = 68.031540000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cliente')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 9.559060000000000000
          Top = 77.692950000000000000
          Width = 90.708720000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor produto')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Left = 230.551330000000000000
          Top = 77.692950000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor desconto')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          Left = 461.102660000000000000
          Top = 77.692950000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor Total')
          ParentFont = False
        end
        object frxDBDataset1Pec_ValorTotal: TfrxMemoView
          Left = 544.252320000000000000
          Top = 77.692950000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pev_ValorTotal'
          DataSet = DBDS
          DataSetName = 'Principal'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[Principal."Pev_ValorTotal"]')
        end
        object frxDBDataset1Pec_ValorDesconto: TfrxMemoView
          Left = 336.378170000000000000
          Top = 77.692950000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pev_ValorDesconto'
          DataSet = DBDS
          DataSetName = 'Principal'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[Principal."Pev_ValorDesconto"]')
        end
        object frxDBDataset1Pec_ValorProduto: TfrxMemoView
          Left = 105.826840000000000000
          Top = 77.692950000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pev_ValorProduto'
          DataSet = DBDS
          DataSetName = 'Principal'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[Principal."Pev_ValorProduto"]')
        end
        object frxDBDataset1Pec_Codigo: TfrxMemoView
          Left = 64.252010000000000000
          Top = 7.559059999999988000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'pev_codigo'
          DataSet = DBDS
          DataSetName = 'Principal'
          HAlign = haRight
          Memo.UTF8W = (
            '[Principal."pev_codigo"]')
        end
        object frxDBDataset1Pec_Data: TfrxMemoView
          Left = 196.535560000000000000
          Top = 7.559059999999988000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'pev_data'
          DataSet = DBDS
          DataSetName = 'Principal'
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Memo.UTF8W = (
            '[Principal."pev_data"]')
        end
        object frxDBDataset1Pec_Baixa: TfrxMemoView
          Left = 332.598640000000000000
          Top = 7.559059999999988000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pev_Baixa'
          DataSet = DBDS
          DataSetName = 'Principal'
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Memo.UTF8W = (
            '[Principal."Pev_Baixa"]')
        end
        object frxDBDataset1Pes_Nome: TfrxMemoView
          Left = 81.708720000000000000
          Top = 32.881892200000010000
          Width = 582.047620000000000000
          Height = 18.897650000000000000
          DataField = 'ClienteNome'
          DataSet = DBDS
          DataSetName = 'Principal'
          Memo.UTF8W = (
            '[Principal."ClienteNome"]')
        end
        object Memo18: TfrxMemoView
          Left = 7.559060000000000000
          Top = 56.826827799999990000
          Width = 68.031540000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Vendedor')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          Left = 83.708720000000000000
          Top = 56.692949999999990000
          Width = 582.047620000000000000
          Height = 18.897650000000000000
          DataField = 'VendedorNome'
          DataSet = DBDS
          DataSetName = 'Principal'
          Memo.UTF8W = (
            '[Principal."VendedorNome"]')
        end
        object Memo20: TfrxMemoView
          Left = 566.929500000000000000
          Top = 7.559059999999988000
          Width = 60.472480000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Situa'#231#227'o')
          ParentFont = False
        end
        object frxDBDataset1Status: TfrxMemoView
          Left = 632.961040000000000000
          Top = 7.559059999999988000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Status'
          DataSet = DBDS
          DataSetName = 'Principal'
          Memo.UTF8W = (
            '[Principal."Status"]')
        end
        object Memo3: TfrxMemoView
          Left = 11.338590000000000000
          Top = 119.976500000000000000
          Width = 54.803149606299200000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Produto')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 411.968503937008000000
          Top = 116.196970000000000000
          Width = 102.047244094488000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Pre'#231'o Unit'#225'rio')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 517.795275590551000000
          Top = 116.196970000000000000
          Width = 79.370078740157500000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Quantidade')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 600.944881889764000000
          Top = 116.196970000000000000
          Width = 109.606299212598000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Valor Total')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Width = 718.110700000000000000
          Color = clBlack
          Diagonal = True
        end
      end
      object DetailData1: TfrxDetailData
        FillType = ftBrush
        Height = 34.015770000000000000
        Top = 347.716760000000000000
        Width = 718.110700000000000000
        DataSet = DBITEM
        DataSetName = 'ITEM'
        RowCount = 0
        object Memo7: TfrxMemoView
          Align = baClient
          Width = 718.110700000000000000
          Height = 34.015770000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Condition = '<line> mod 2 = 1'
          Highlight.FillType = ftBrush
          Highlight.Fill.BackColor = clSilver
        end
        object ITEMpci_produto: TfrxMemoView
          Left = 7.559060000000000000
          Top = 7.559059999999988000
          Width = 54.803149606299200000
          Height = 18.897650000000000000
          DataField = 'pvi_produto'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pvi_produto"]')
        end
        object ITEMpro_descricao: TfrxMemoView
          Left = 64.252010000000000000
          Top = 7.559059999999988000
          Width = 343.937230000000000000
          Height = 18.897650000000000000
          DataField = 'pro_descricao'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          Memo.UTF8W = (
            '[ITEM."pro_descricao"]')
        end
        object ITEMpci_valortotal: TfrxMemoView
          Left = 600.944881889764000000
          Top = 7.559059999999988000
          Width = 109.606299212598000000
          Height = 18.897650000000000000
          DataField = 'pvi_valortotal'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pvi_valortotal"]')
        end
        object ITEMpci_quantidade: TfrxMemoView
          Left = 517.795275590551000000
          Top = 7.559059999999988000
          Width = 79.370078740157500000
          Height = 18.897650000000000000
          DataField = 'pvi_quantidade'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pvi_quantidade"]')
        end
        object ITEMpci_precounitario: TfrxMemoView
          Left = 411.968503937008000000
          Top = 7.559059999999988000
          Width = 102.047244094488000000
          Height = 18.897650000000000000
          DataField = 'pvi_precounitario'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pvi_precounitario"]')
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        Height = 22.677180000000000000
        Top = 442.205010000000000000
        Width = 718.110700000000000000
        object Memo8: TfrxMemoView
          Left = 476.205010000000000000
          Top = 3.779527560000020000
          Width = 170.078850000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Quantidade de venda - >')
          ParentFont = False
        end
        object Line4: TfrxLineView
          Align = baCenter
          Left = -0.000000000000024203
          Width = 718.110700000000000000
          Color = clBlack
          Diagonal = True
        end
        object SysMemo1: TfrxSysMemoView
          Left = 650.622450000000000000
          Top = 3.779530000000022000
          Width = 64.252010000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            '[COUNT(MasterData1)]')
          ParentFont = False
        end
      end
    end
  end
  object Pedido: TFDQuery
    IndexFieldNames = 'pev_codigo'
    Connection = D.FDCon
    SQL.Strings = (
      
        'select pev_codigo,pev_data,Pev_Baixa,Pev_Cliente,Pev_Vendedor, i' +
        'if(Pev_Status = '#39'P'#39','#39'Pendente'#39','#39'Baixado'#39') as Status,Pev_ValorTot' +
        'al,Pev_ValorDesconto, Pev_ValorProduto,'
      
        'Pcl.Pes_nome as ClienteNome, Pve.Pes_Nome as VendedorNome from p' +
        'edidovenda pv'
      'inner join pessoa Pcl on Pcl.Pes_codigo = pv.Pev_Cliente'
      'inner join pessoa Pve on Pve.Pes_codigo = pv.Pev_Vendedor')
    Left = 80
    Top = 60
    object Pedidopev_codigo: TFDAutoIncField
      FieldName = 'pev_codigo'
      Origin = 'pev_codigo'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object Pedidopev_data: TDateField
      FieldName = 'pev_data'
      Origin = 'pev_data'
      Required = True
    end
    object PedidoPev_Baixa: TDateField
      FieldName = 'Pev_Baixa'
      Origin = 'Pev_Baixa'
    end
    object PedidoPev_Cliente: TIntegerField
      FieldName = 'Pev_Cliente'
      Origin = 'Pev_Cliente'
      Required = True
    end
    object PedidoPev_Vendedor: TIntegerField
      FieldName = 'Pev_Vendedor'
      Origin = 'Pev_Vendedor'
    end
    object PedidoStatus: TStringField
      FieldName = 'Status'
      Origin = 'Status'
      ReadOnly = True
      Required = True
      Size = 8
    end
    object PedidoPev_ValorTotal: TBCDField
      FieldName = 'Pev_ValorTotal'
      Origin = 'Pev_ValorTotal'
      Precision = 14
      Size = 2
    end
    object PedidoPev_ValorDesconto: TBCDField
      FieldName = 'Pev_ValorDesconto'
      Origin = 'Pev_ValorDesconto'
      Precision = 14
      Size = 2
    end
    object PedidoPev_ValorProduto: TBCDField
      FieldName = 'Pev_ValorProduto'
      Origin = 'Pev_ValorProduto'
      Precision = 14
      Size = 2
    end
    object PedidoClienteNome: TStringField
      FieldName = 'ClienteNome'
      Origin = 'ClienteNome'
      Size = 50
    end
    object PedidoVendedorNome: TStringField
      FieldName = 'VendedorNome'
      Origin = 'VendedorNome'
      Size = 50
    end
  end
  object Q: TFDQuery
    IndexFieldNames = 'pvi_pedidovenda;pvi_codigo'
    MasterSource = DS
    MasterFields = 'pev_codigo'
    Connection = D.FDCon
    FetchOptions.AssignedValues = [evCache]
    FetchOptions.Cache = [fiBlobs, fiMeta]
    SQL.Strings = (
      
        'select pvi_pedidovenda,pvi_codigo,pvi_produto,pro_descricao,pvi_' +
        'precounitario, pvi_quantidade, pvi_valortotal from pedidovendait' +
        'em'
      
        'inner join produto on produto.pro_codigo = pedidovendaitem.pvi_p' +
        'roduto')
    Left = 48
    Top = 60
    object Qpvi_pedidovenda: TIntegerField
      FieldName = 'pvi_pedidovenda'
      Origin = 'pvi_pedidovenda'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object Qpvi_produto: TIntegerField
      FieldName = 'pvi_produto'
      Origin = 'pvi_produto'
      Required = True
    end
    object Qpro_descricao: TStringField
      FieldName = 'pro_descricao'
      Origin = 'pro_descricao'
      Required = True
      Size = 50
    end
    object Qpvi_precounitario: TBCDField
      FieldName = 'pvi_precounitario'
      Origin = 'pvi_precounitario'
      Precision = 14
      Size = 2
    end
    object Qpvi_quantidade: TBCDField
      FieldName = 'pvi_quantidade'
      Origin = 'pvi_quantidade'
      Precision = 14
      Size = 2
    end
    object Qpvi_valortotal: TBCDField
      FieldName = 'pvi_valortotal'
      Origin = 'pvi_valortotal'
      Precision = 14
      Size = 2
    end
    object Qpvi_codigo: TIntegerField
      FieldName = 'pvi_codigo'
      Origin = 'pvi_codigo'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
  end
  object DS: TDataSource
    DataSet = Pedido
    Left = 16
    Top = 60
  end
  object DBITEM: TfrxDBDataset
    UserName = 'ITEM'
    CloseDataSource = False
    FieldAliases.Strings = (
      'pvi_pedidovenda=pvi_pedidovenda'
      'pvi_codigo=pvi_codigo'
      'pvi_produto=pvi_produto'
      'pro_descricao=pro_descricao'
      'pvi_precounitario=pvi_precounitario'
      'pvi_quantidade=pvi_quantidade'
      'pvi_valortotal=pvi_valortotal')
    DataSet = Item
    BCDToCurrency = False
    Left = 416
    Top = 152
  end
  object DBDS: TfrxDBDataset
    UserName = 'Principal'
    CloseDataSource = False
    FieldAliases.Strings = (
      'pev_codigo=pev_codigo'
      'pev_data=pev_data'
      'Pev_Baixa=Pev_Baixa'
      'Pev_Cliente=Pev_Cliente'
      'Pev_Vendedor=Pev_Vendedor'
      'Status=Status'
      'Pev_ValorTotal=Pev_ValorTotal'
      'Pev_ValorDesconto=Pev_ValorDesconto'
      'Pev_ValorProduto=Pev_ValorProduto'
      'ClienteNome=ClienteNome'
      'VendedorNome=VendedorNome')
    DataSet = Pedido
    BCDToCurrency = False
    Left = 376
    Top = 152
  end
  object Item: TFDQuery
    IndexFieldNames = 'pvi_pedidovenda;pvi_codigo'
    MasterSource = DS
    MasterFields = 'pev_codigo'
    Connection = D.FDCon
    SQL.Strings = (
      
        'select pvi_pedidovenda,pvi_codigo,pvi_produto,pro_descricao,pvi_' +
        'precounitario, pvi_quantidade, pvi_valortotal from pedidovendait' +
        'em'
      
        'inner join produto on produto.pro_codigo = pedidovendaitem.pvi_p' +
        'roduto')
    Left = 112
    Top = 60
    object Itempvi_pedidovenda: TIntegerField
      FieldName = 'pvi_pedidovenda'
      Origin = 'pvi_pedidovenda'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object Itempvi_codigo: TIntegerField
      FieldName = 'pvi_codigo'
      Origin = 'pvi_codigo'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object Itempvi_produto: TIntegerField
      FieldName = 'pvi_produto'
      Origin = 'pvi_produto'
      Required = True
    end
    object Itempro_descricao: TStringField
      FieldName = 'pro_descricao'
      Origin = 'pro_descricao'
      Required = True
      Size = 50
    end
    object Itempvi_precounitario: TBCDField
      FieldName = 'pvi_precounitario'
      Origin = 'pvi_precounitario'
      Precision = 14
      Size = 2
    end
    object Itempvi_quantidade: TBCDField
      FieldName = 'pvi_quantidade'
      Origin = 'pvi_quantidade'
      Precision = 14
      Size = 2
    end
    object Itempvi_valortotal: TBCDField
      FieldName = 'pvi_valortotal'
      Origin = 'pvi_valortotal'
      Precision = 14
      Size = 2
    end
  end
end
