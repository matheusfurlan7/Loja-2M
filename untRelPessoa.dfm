inherited FrmRelPessoa: TFrmRelPessoa
  Caption = ''
  ClientHeight = 477
  ClientWidth = 605
  ExplicitWidth = 611
  ExplicitHeight = 506
  PixelsPerInch = 96
  TextHeight = 13
  inherited PTopo: TPanel
    Width = 605
    ExplicitWidth = 500
    inherited LCad: TLabel
      Width = 191
      Caption = 'Relat'#243'rio - Pessoa'
      ExplicitWidth = 191
    end
  end
  inherited PF1: TPanel
    Width = 605
    Height = 404
    ExplicitWidth = 500
    ExplicitHeight = 472
    object Label6: TLabel
      Left = 281
      Top = 198
      Width = 41
      Height = 13
      Caption = 'Situa'#231#227'o'
    end
    object Label7: TLabel
      Left = 417
      Top = 171
      Width = 24
      Height = 13
      Caption = 'Sexo'
    end
    object Label22: TLabel
      Left = 37
      Top = 171
      Width = 33
      Height = 13
      Caption = 'Cliente'
    end
    object Label23: TLabel
      Left = 15
      Top = 198
      Width = 55
      Height = 13
      Caption = 'Fornecedor'
    end
    object Label24: TLabel
      Left = 164
      Top = 171
      Width = 46
      Height = 13
      Caption = 'Vendedor'
    end
    object Label25: TLabel
      Left = 157
      Top = 198
      Width = 53
      Height = 13
      Caption = 'Comprador'
    end
    object Label5: TLabel
      Left = 302
      Top = 171
      Width = 20
      Height = 13
      Caption = 'Tipo'
    end
    object GCod: TGroupBox
      Left = 20
      Top = 9
      Width = 107
      Height = 75
      Caption = 'C'#243'digo'
      TabOrder = 0
      object De: TLabel
        Left = 16
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Até: TLabel
        Left = 12
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ECodDe: TEdit
        Left = 35
        Top = 18
        Width = 61
        Height = 21
        NumbersOnly = True
        TabOrder = 0
      end
      object ECodAte: TEdit
        Left = 35
        Top = 45
        Width = 61
        Height = 21
        NumbersOnly = True
        TabOrder = 1
      end
    end
    object GNome: TGroupBox
      Left = 133
      Top = 9
      Width = 347
      Height = 75
      Caption = 'Nome'
      TabOrder = 1
      object Label1: TLabel
        Left = 9
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label2: TLabel
        Left = 5
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ENomeDe: TEdit
        Left = 28
        Top = 18
        Width = 310
        Height = 21
        MaxLength = 50
        TabOrder = 0
      end
      object ENomeAte: TEdit
        Left = 28
        Top = 45
        Width = 310
        Height = 21
        MaxLength = 50
        TabOrder = 1
      end
    end
    object GRazao: TGroupBox
      Left = 20
      Top = 90
      Width = 347
      Height = 75
      Caption = 'Raz'#227'o social'
      TabOrder = 3
      object Label3: TLabel
        Left = 9
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label4: TLabel
        Left = 5
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ERazaoDe: TEdit
        Left = 28
        Top = 18
        Width = 310
        Height = 21
        MaxLength = 50
        TabOrder = 0
      end
      object ERazaoAte: TEdit
        Left = 28
        Top = 45
        Width = 310
        Height = 21
        MaxLength = 50
        TabOrder = 1
      end
    end
    object ESituacao: TComboBox
      Left = 328
      Top = 198
      Width = 70
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 8
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Ativo'
        'Inativo')
    end
    object ESexo: TComboBox
      Left = 447
      Top = 171
      Width = 70
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 9
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Masculino'
        'Feminino')
    end
    object GCadastro: TGroupBox
      Left = 486
      Top = 9
      Width = 107
      Height = 75
      Caption = 'Cadastro'
      TabOrder = 2
      object Label8: TLabel
        Left = 16
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label9: TLabel
        Left = 12
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ECadastroDe: TMaskEdit
        Left = 35
        Top = 18
        Width = 61
        Height = 21
        EditMask = '!99/99/9999;1;_'
        MaxLength = 10
        TabOrder = 0
        Text = '  /  /    '
        OnExit = ECadastroDeExit
      end
      object ECadastroAte: TMaskEdit
        Left = 35
        Top = 45
        Width = 61
        Height = 21
        EditMask = '!99/99/9999;1;_'
        MaxLength = 10
        TabOrder = 1
        Text = '  /  /    '
        OnExit = ECadastroAteExit
      end
    end
    object GNascimento: TGroupBox
      Left = 373
      Top = 90
      Width = 106
      Height = 75
      Caption = 'Data de nascimento'
      TabOrder = 10
      object Label10: TLabel
        Left = 16
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label11: TLabel
        Left = 12
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ENascimentoDe: TMaskEdit
        Left = 35
        Top = 18
        Width = 61
        Height = 21
        EditMask = '!99/99/9999;1;_'
        MaxLength = 10
        TabOrder = 0
        Text = '  /  /    '
        OnExit = ENascimentoDeExit
      end
      object ENascimentoAte: TMaskEdit
        Left = 35
        Top = 45
        Width = 61
        Height = 21
        EditMask = '!99/99/9999;1;_'
        MaxLength = 10
        TabOrder = 1
        Text = '  /  /    '
        OnExit = ENascimentoAteExit
      end
    end
    object GPais: TGroupBox
      Left = 20
      Top = 225
      Width = 460
      Height = 75
      Caption = 'Pa'#237's'
      TabOrder = 11
      object Label16: TLabel
        Left = 11
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label17: TLabel
        Left = 7
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object CPaisDe: TView
        Left = 88
        Top = 18
        Width = 367
        Height = 21
        Margins.Right = 10
        OnActive = CPaisDeActive
        Connection = D.FDCon
        ShowCaption = True
        Associate = EPaisDe
        Color = clWindow
      end
      object CPaisAte: TView
        Left = 88
        Top = 45
        Width = 367
        Height = 21
        Margins.Right = 10
        OnActive = CPaisAteActive
        Connection = D.FDCon
        ShowCaption = True
        Associate = EPaisAte
        Color = clWindow
      end
      object EPaisDe: TEdit
        Left = 30
        Top = 18
        Width = 56
        Height = 21
        NumbersOnly = True
        TabOrder = 2
        OnEnter = EPaisDeEnter
        OnExit = EPaisDeExit
      end
      object EPaisAte: TEdit
        Left = 30
        Top = 45
        Width = 56
        Height = 21
        NumbersOnly = True
        TabOrder = 3
        OnEnter = EPaisAteEnter
        OnExit = EPaisAteExit
      end
    end
    object GEstado: TGroupBox
      Left = 20
      Top = 306
      Width = 113
      Height = 75
      Caption = 'Estado'
      TabOrder = 12
      object Label18: TLabel
        Left = 11
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label19: TLabel
        Left = 7
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object EEstadoDe: TMaskEdit
        Left = 30
        Top = 18
        Width = 48
        Height = 21
        CharCase = ecUpperCase
        DoubleBuffered = False
        MaxLength = 2
        ParentDoubleBuffered = False
        TabOrder = 0
        Text = ''
        OnEnter = EEstadoDeEnter
        OnExit = EEstadoDeExit
      end
      object EEstadoAte: TMaskEdit
        Left = 30
        Top = 45
        Width = 48
        Height = 21
        CharCase = ecUpperCase
        DoubleBuffered = False
        MaxLength = 2
        ParentDoubleBuffered = False
        TabOrder = 1
        Text = ''
        OnEnter = EEstadoAteEnter
        OnExit = EEstadoAteExit
      end
      object CEstadoDe: TView
        Left = 79
        Top = 18
        Width = 24
        Height = 21
        Margins.Right = 10
        OnActive = CEstadoDeActive
        Connection = D.FDCon
        ShowCaption = False
        Associate = EEstadoDe
        Color = clWindow
      end
      object CEstadoAte: TView
        Left = 79
        Top = 45
        Width = 24
        Height = 21
        Margins.Right = 10
        OnActive = CEstadoAteActive
        Connection = D.FDCon
        ShowCaption = False
        Associate = EEstadoAte
        Color = clWindow
      end
    end
    object GCidade: TGroupBox
      Left = 171
      Top = 306
      Width = 309
      Height = 75
      Caption = 'Cidade'
      TabOrder = 13
      object Label20: TLabel
        Left = 9
        Top = 18
        Width = 13
        Height = 13
        Caption = 'De'
      end
      object Label21: TLabel
        Left = 5
        Top = 45
        Width = 17
        Height = 13
        Caption = 'At'#233
      end
      object ECidadeDe: TMaskEdit
        Left = 28
        Top = 18
        Width = 249
        Height = 21
        TabOrder = 0
        Text = ''
        OnExit = ECidadeDeExit
      end
      object ECidadeAte: TMaskEdit
        Left = 28
        Top = 45
        Width = 249
        Height = 21
        TabOrder = 1
        Text = ''
        OnExit = ECidadeAteExit
      end
      object CCidadeDe: TView
        Left = 280
        Top = 18
        Width = 24
        Height = 21
        Margins.Right = 10
        OnActive = CCidadeDeActive
        Connection = D.FDCon
        ShowCaption = False
        Associate = ECidadeDe
        Color = clWindow
      end
      object CCidadeAte: TView
        Left = 280
        Top = 45
        Width = 24
        Height = 21
        Margins.Right = 10
        OnActive = CCidadeAteActive
        Connection = D.FDCon
        ShowCaption = False
        Associate = ECidadeAte
        Color = clWindow
      end
    end
    object ECliente: TComboBox
      Left = 76
      Top = 171
      Width = 57
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 4
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Sim'
        'N'#227'o')
    end
    object EFornecedor: TComboBox
      Left = 76
      Top = 198
      Width = 57
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 5
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Sim'
        'N'#227'o')
    end
    object EVendedor: TComboBox
      Left = 216
      Top = 171
      Width = 57
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 6
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Sim'
        'N'#227'o')
    end
    object EComprador: TComboBox
      Left = 216
      Top = 198
      Width = 57
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 7
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'Sim'
        'N'#227'o')
    end
    object ETipo: TComboBox
      Left = 328
      Top = 171
      Width = 78
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 14
      Text = 'Ambos'
      Items.Strings = (
        'Ambos'
        'F'#237'sica'
        'Jur'#237'dica')
    end
  end
  inherited ToolBar1: TToolBar
    Top = 440
    Width = 605
    ExplicitTop = 508
    ExplicitWidth = 500
  end
  inherited Relatorio: TfrxReport
    ReportOptions.LastChange = 43607.878388020800000000
    Datasets = <
      item
        DataSet = DBDS
        DataSetName = 'frxDBDataset1'
      end>
    Variables = <>
    Style = <>
    inherited Page1: TfrxReportPage
      inherited MasterData1: TfrxMasterData
        Height = 177.637910000000000000
        inherited Memo8: TfrxMemoView
          Height = 177.637910000000000000
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Left = 3.779530000000000000
          Top = 11.338590000000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'C'#243'digo')
          ParentFont = False
        end
        object frxDBDataset1Pes_Codigo: TfrxMemoView
          Left = 54.472480000000000000
          Top = 11.338590000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Codigo'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Codigo"]')
        end
        object Memo6: TfrxMemoView
          Left = 3.779530000000000000
          Top = 34.015770000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Nome')
          ParentFont = False
        end
        object frxDBDataset1Pes_Nome: TfrxMemoView
          Left = 49.133890000000000000
          Top = 34.015770000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Nome'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Nome"]')
        end
        object Memo9: TfrxMemoView
          Left = 3.779530000000000000
          Top = 56.692950000000000000
          Width = 86.929190000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Raz'#227'o social')
          ParentFont = False
        end
        object frxDBDataset1Pes_RazaoSocial: TfrxMemoView
          Left = 92.606370000000000000
          Top = 56.692950000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_RazaoSocial'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_RazaoSocial"]')
        end
        object Memo10: TfrxMemoView
          Left = 570.709030000000000000
          Top = 11.338590000000000000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Situa'#231#227'o')
          ParentFont = False
        end
        object frxDBDataset1Pes_Situacao: TfrxMemoView
          Left = 631.181510000000000000
          Top = 11.338585120000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Situacao'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Situacao"]')
        end
        object Memo11: TfrxMemoView
          Left = 3.779530000000000000
          Top = 79.267780000000000000
          Width = 37.795300000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Tipo')
          ParentFont = False
        end
        object frxDBDataset1Pes_TipoDePessoa: TfrxMemoView
          Left = 45.354360000000000000
          Top = 79.267780000000000000
          Width = 86.929133860000000000
          Height = 18.897650000000000000
          DataField = 'Pes_TipoDePessoa'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_TipoDePessoa"]')
        end
        object Memo12: TfrxMemoView
          Left = 136.283550000000000000
          Top = 79.370073860000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CPF/CNPJ')
          ParentFont = False
        end
        object frxDBDataset1Pes_CPFCNPJ: TfrxMemoView
          Left = 210.874150000000000000
          Top = 79.370073860000000000
          Width = 143.622140000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_CPFCNPJ'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_CPFCNPJ"]')
        end
        object Memo13: TfrxMemoView
          Left = 358.275820000000000000
          Top = 79.370073860000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'RG/IE')
          ParentFont = False
        end
        object frxDBDataset1Pes_RGIE: TfrxMemoView
          Left = 407.409710000000000000
          Top = 79.370073860000000000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_RGIE'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_RGIE"]')
        end
        object Memo14: TfrxMemoView
          Left = 487.338900000000000000
          Top = 34.015765120000000000
          Width = 60.472480000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Telefone')
          ParentFont = False
        end
        object frxDBDataset1Pes_Telefone: TfrxMemoView
          Left = 551.590910000000000000
          Top = 34.015770000000000000
          Width = 162.519790000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Telefone'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Telefone"]')
        end
        object Memo15: TfrxMemoView
          Left = 498.677490000000000000
          Top = 56.692945120000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Celular')
          ParentFont = False
        end
        object frxDBDataset1Pes_Celular: TfrxMemoView
          Left = 551.590910000000000000
          Top = 56.692950000000000000
          Width = 162.519790000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Celular'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Celular"]')
        end
        object frxDBDataset1Pes_Cliente: TfrxMemoView
          Left = 65.031540000000000000
          Top = 103.937007870000000000
          Width = 45.354330708661400000
          Height = 18.897650000000000000
          DataField = 'Pes_Cliente'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Cliente"]')
        end
        object Memo16: TfrxMemoView
          Left = 3.779530000000000000
          Top = 103.826840000000000000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cliente')
          ParentFont = False
        end
        object frxDBDataset1Pes_Fornecedor: TfrxMemoView
          Left = 214.094620000000000000
          Top = 103.937007870000000000
          Width = 45.354330710000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Fornecedor'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Fornecedor"]')
        end
        object Memo17: TfrxMemoView
          Left = 115.165430000000000000
          Top = 103.937007870000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Fornecedor')
          ParentFont = False
        end
        object Memo18: TfrxMemoView
          Left = 265.110390000000000000
          Top = 103.937007874016000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Vendedor')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          Left = 414.953000000000000000
          Top = 103.937007870000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Comprador')
          ParentFont = False
        end
        object frxDBDataset1Pes_Vendedor: TfrxMemoView
          Left = 363.378170000000000000
          Top = 103.937007870000000000
          Width = 45.354330708661400000
          Height = 18.897650000000000000
          DataField = 'Pes_Vendedor'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Vendedor"]')
        end
        object frxDBDataset1Pes_Comprador: TfrxMemoView
          Left = 513.220780000000000000
          Top = 103.937007870000000000
          Width = 45.354330710000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Comprador'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Comprador"]')
        end
        object Memo20: TfrxMemoView
          Left = 147.401670000000000000
          Top = 11.338590000000000000
          Width = 37.795300000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Sexo')
          ParentFont = False
        end
        object frxDBDataset1Pes_Sexo: TfrxMemoView
          Left = 192.756030000000000000
          Top = 11.338590000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Sexo'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Sexo"]')
        end
        object Memo21: TfrxMemoView
          Left = 3.779530000000000000
          Top = 127.504020000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Pa'#237's')
          ParentFont = False
        end
        object frxDBDataset1Pai_Nome: TfrxMemoView
          Left = 124.724490000000000000
          Top = 127.504020000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'Pai_Nome'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pai_Nome"]')
        end
        object frxDBDataset1Pes_Pais: TfrxMemoView
          Left = 41.574830000000000000
          Top = 127.504020000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Pais"]')
        end
        object Memo22: TfrxMemoView
          Left = 3.779530000000000000
          Top = 151.960730000000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Estado')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Left = 105.826840000000000000
          Top = 151.960730000000000000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Cidade')
          ParentFont = False
        end
        object frxDBDataset1Pes_Estado: TfrxMemoView
          Left = 56.692950000000000000
          Top = 151.960730000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Estado'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Estado"]')
        end
        object frxDBDataset1Pes_Cidade: TfrxMemoView
          Left = 158.740260000000000000
          Top = 151.960730000000000000
          Width = 321.260050000000000000
          Height = 18.897650000000000000
          DataField = 'Pes_Cidade'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Pes_Cidade"]')
        end
      end
      inherited PageFooter1: TfrxPageFooter
        Top = 506.457020000000000000
        inherited Memo1: TfrxMemoView
          Left = 537.016080000000000000
          Width = 177.637910000000000000
          Formats = <
            item
            end
            item
            end>
        end
        inherited Line1: TfrxLineView
          Top = 1.779530000000140000
        end
      end
      inherited PageHeader1: TfrxPageHeader
        inherited Picture1: TfrxPictureView
          Picture.Data = {
            
            38B22E7D0000000049454E44AE426082}
        end
      end
      inherited ReportSummary1: TfrxReportSummary
        Top = 461.102660000000000000
        inherited Memo7: TfrxMemoView
          Top = 3.779527560000020000
        end
        inherited SysMemo1: TfrxSysMemoView
          Top = 3.779530000000020000
        end
      end
    end
  end
  inherited DBDS: TfrxDBDataset
    FieldAliases.Strings = (
      'Pes_Codigo=Pes_Codigo'
      'Pes_Nome=Pes_Nome'
      'Pes_RazaoSocial=Pes_RazaoSocial'
      'Pes_Situacao=Pes_Situacao'
      'Pes_TipoDePessoa=Pes_TipoDePessoa'
      'Pes_CPFCNPJ=Pes_CPFCNPJ'
      'Pes_RGIE=Pes_RGIE'
      'Pes_Telefone=Pes_Telefone'
      'Pes_Celular=Pes_Celular'
      'Pes_Email=Pes_Email'
      'Pes_Site=Pes_Site'
      'Pes_Pais=Pes_Pais'
      'Pes_Estado=Pes_Estado'
      'Pes_Cidade=Pes_Cidade'
      'Pes_Bairro=Pes_Bairro'
      'Pes_Endereco=Pes_Endereco'
      'Pes_Numero=Pes_Numero'
      'Pes_Complemento=Pes_Complemento'
      'Pes_Cep=Pes_Cep'
      'Pes_DataCadastro=Pes_DataCadastro'
      'Pes_DataNascimento=Pes_DataNascimento'
      'Pes_Cliente=Pes_Cliente'
      'Pes_Fornecedor=Pes_Fornecedor'
      'Pes_Sexo=Pes_Sexo'
      'Pes_Vendedor=Pes_Vendedor'
      'Pes_Comprador=Pes_Comprador'
      'Pai_Nome=Pai_Nome')
  end
  inherited Q: TFDQuery
    SQL.Strings = (
      
        'SELECT Pes_Codigo, Pes_Nome, Pes_RazaoSocial, Pes_Situacao, Pes_' +
        'TipoDePessoa,'
      
        'Pes_CPFCNPJ, Pes_RGIE, Pes_Telefone, Pes_Celular, Pes_Email, Pes' +
        '_Site, Pes_Pais,'
      
        'Pes_Estado, Pes_Cidade, Pes_Bairro, Pes_Endereco, Pes_Numero, Pe' +
        's_Complemento,'
      
        'Pes_Cep, Pes_DataCadastro, Pes_DataNascimento, Pes_Cliente, Pes_' +
        'Fornecedor,'
      'Pes_Sexo, Pes_Vendedor, Pes_Comprador, Pai_Nome FROM Pessoa'
      'INNER JOIN Pais on (Pai_Codigo = Pes_Pais)')
  end
end
