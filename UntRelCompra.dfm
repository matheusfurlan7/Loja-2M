inherited FrmRelCompra: TFrmRelCompra
  Caption = 'FrmRelCompra'
  ExplicitWidth = 651
  ExplicitHeight = 338
  PixelsPerInch = 96
  TextHeight = 13
  inherited PTopo: TPanel
    inherited LCad: TLabel
      Width = 639
      Height = 30
    end
  end
  inherited Relatorio: TfrxReport
    ReportOptions.LastChange = 43611.681724247700000000
    Datasets = <
      item
        DataSet = DBDS
        DataSetName = 'frxDBDataset1'
      end
      item
        DataSet = DBITEM
        DataSetName = 'ITEM'
      end>
    Variables = <>
    Style = <>
    inherited Page1: TfrxReportPage
      inherited MasterData1: TfrxMasterData
        Height = 136.063080000000000000
        Top = 177.637910000000000000
        Width = 718.110700000000000000
        inherited Memo8: TfrxMemoView
          Width = 718.110700000000000000
          Height = 136.063080000000000000
          HAlign = haRight
        end
        object Memo11: TfrxMemoView
          Left = 3.779530000000000000
          Top = 3.779529999999994000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'C'#243'digo')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 143.622140000000000000
          Top = 3.779529999999994000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Data')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 283.464750000000000000
          Top = 3.779529999999994000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Baixa')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          Left = 3.779530000000000000
          Top = 29.236240000000010000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Fornecedor')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          Left = 3.779530000000000000
          Top = 73.913420000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor produto')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Left = 226.771800000000000000
          Top = 73.913420000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor desconto')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          Left = 457.323130000000000000
          Top = 73.913420000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Valor Total')
          ParentFont = False
        end
        object frxDBDataset1Pec_ValorTotal: TfrxMemoView
          Left = 540.472790000000000000
          Top = 73.913420000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_ValorTotal'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_ValorTotal"]')
        end
        object frxDBDataset1Pec_ValorDesconto: TfrxMemoView
          Left = 332.598640000000000000
          Top = 73.913420000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_ValorDesconto'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_ValorDesconto"]')
        end
        object frxDBDataset1Pec_ValorProduto: TfrxMemoView
          Left = 102.047310000000000000
          Top = 73.913420000000000000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_ValorProduto'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_ValorProduto"]')
        end
        object frxDBDataset1Pec_Codigo: TfrxMemoView
          Left = 60.472480000000000000
          Top = 3.779529999999994000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_Codigo'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_Codigo"]')
        end
        object frxDBDataset1Pec_Data: TfrxMemoView
          Left = 192.756030000000000000
          Top = 3.779529999999994000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_Data'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_Data"]')
        end
        object frxDBDataset1Pec_Baixa: TfrxMemoView
          Left = 328.819110000000000000
          Top = 3.779529999999994000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DataField = 'Pec_Baixa'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Memo.UTF8W = (
            '[frxDBDataset1."Pec_Baixa"]')
        end
        object frxDBDataset1Pes_Nome: TfrxMemoView
          Left = 86.929190000000000000
          Top = 29.102362199999990000
          Width = 574.488560000000000000
          Height = 18.897650000000000000
          DataField = 'FornecedorNome'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."FornecedorNome"]')
        end
        object Memo18: TfrxMemoView
          Left = 3.779530000000000000
          Top = 53.047297799999990000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Comprador')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          Left = 86.929190000000000000
          Top = 52.913420000000000000
          Width = 574.488560000000000000
          Height = 18.897650000000000000
          DataField = 'CompradorNome'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."CompradorNome"]')
        end
        object Memo20: TfrxMemoView
          Left = 559.370440000000000000
          Top = 3.779529999999994000
          Width = 64.252010000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Situa'#231#227'o')
          ParentFont = False
        end
        object frxDBDataset1Status: TfrxMemoView
          Left = 631.181510000000000000
          Top = 3.779529999999994000
          Width = 83.149660000000000000
          Height = 18.897650000000000000
          DataField = 'Status'
          DataSet = DBDS
          DataSetName = 'frxDBDataset1'
          Memo.UTF8W = (
            '[frxDBDataset1."Status"]')
        end
        object Memo3: TfrxMemoView
          Left = 3.779527560000000000
          Top = 109.944881889764000000
          Width = 79.370078740000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Produto')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 411.968503940000000000
          Top = 109.944881889764000000
          Width = 102.047244090000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Pre'#231'o Unit'#225'rio')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 517.795275590000000000
          Top = 109.944881889764000000
          Width = 79.370078740000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Quantidade')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 600.944881890000000000
          Top = 109.944881889764000000
          Width = 109.606299210000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Valor Total')
          ParentFont = False
        end
        object Line4: TfrxLineView
          Align = baCenter
          Left = -0.000000000000024203
          Width = 718.110700000000000000
          Color = clBlack
          Diagonal = True
        end
      end
      inherited PageFooter1: TfrxPageFooter
        Top = 514.016080000000000000
        Width = 718.110700000000000000
        inherited Memo1: TfrxMemoView
          Top = 7.000000000000114000
        end
        inherited Line1: TfrxLineView
          Top = 1.779530000000022000
          Width = 718.110700000000000000
        end
      end
      inherited PageHeader1: TfrxPageHeader
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        inherited Picture1: TfrxPictureView
          Top = 7.559059999999999000
          Picture.Data = {
            
            38B22E7D0000000049454E44AE426082}
        end
        inherited Memo4: TfrxMemoView
          Left = 255.118275000000000000
          Top = 7.559059999999999000
        end
        inherited Memo5: TfrxMemoView
          Left = 308.031694999999900000
          Top = 64.252010000000000000
        end
        inherited Line2: TfrxLineView
          Left = -0.000000000000024203
          Top = 98.267780000000000000
        end
        inherited Memo2: TfrxMemoView
          Left = 582.047620000000000000
          Top = 71.811070000000000000
        end
      end
      inherited ReportSummary1: TfrxReportSummary
        Top = 468.661720000000000000
        Width = 718.110700000000000000
        inherited Memo7: TfrxMemoView
          Top = 3.779527560000020000
        end
        inherited Line3: TfrxLineView
          Left = -0.000000000000024203
        end
        inherited SysMemo1: TfrxSysMemoView
          Top = 3.779530000000022000
        end
      end
      inherited Header1: TfrxHeader
        Top = 336.378170000000000000
        Width = 718.110700000000000000
      end
      object DetailData1: TfrxDetailData
        FillType = ftBrush
        Height = 26.456710000000000000
        Top = 381.732530000000000000
        Width = 718.110700000000000000
        DataSet = DBITEM
        DataSetName = 'ITEM'
        RowCount = 0
        object Memo21: TfrxMemoView
          Align = baClient
          Width = 718.110700000000000000
          Height = 26.456710000000000000
          Highlight.Font.Charset = DEFAULT_CHARSET
          Highlight.Font.Color = clRed
          Highlight.Font.Height = -13
          Highlight.Font.Name = 'Arial'
          Highlight.Font.Style = []
          Highlight.Condition = '<line> mod 2 = 1'
          Highlight.FillType = ftBrush
          Highlight.Fill.BackColor = clSilver
        end
        object ITEMpci_produto: TfrxMemoView
          Left = 3.779527559055120000
          Top = 3.779530000000022000
          Width = 79.370078740157500000
          Height = 18.897650000000000000
          DataField = 'pci_produto'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pci_produto"]')
        end
        object ITEMpro_descricao: TfrxMemoView
          Left = 86.929190000000000000
          Top = 3.779530000000022000
          Width = 317.480520000000000000
          Height = 18.897650000000000000
          DataField = 'pro_descricao'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          Memo.UTF8W = (
            '[ITEM."pro_descricao"]')
        end
        object ITEMpci_valortotal: TfrxMemoView
          Left = 600.944881890000000000
          Top = 3.779530000000022000
          Width = 109.606299210000000000
          Height = 18.897650000000000000
          DataField = 'pci_valortotal'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pci_valortotal"]')
        end
        object ITEMpci_quantidade: TfrxMemoView
          Left = 517.795275590000000000
          Top = 3.779530000000022000
          Width = 79.370078740157500000
          Height = 18.897650000000000000
          DataField = 'pci_quantidade'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pci_quantidade"]')
        end
        object ITEMpci_precounitario: TfrxMemoView
          Left = 411.968503937008000000
          Top = 3.779530000000022000
          Width = 102.047244094488000000
          Height = 18.897650000000000000
          DataField = 'pci_precounitario'
          DataSet = DBITEM
          DataSetName = 'ITEM'
          DisplayFormat.FormatStr = '%2.2f'
          DisplayFormat.Kind = fkNumeric
          HAlign = haRight
          Memo.UTF8W = (
            '[ITEM."pci_precounitario"]')
        end
      end
    end
  end
  inherited DBDS: TfrxDBDataset
    FieldAliases.Strings = (
      'pec_codigo=Pec_Codigo'
      'pec_data=Pec_Data'
      'Pec_Baixa=Pec_Baixa'
      'Pec_Fornecedor=Pec_Fornecedor'
      'Pec_Comprador=Pec_Comprador'
      'Status=Status'
      'Pec_ValorTotal=Pec_ValorTotal'
      'Pec_ValorDesconto=Pec_ValorDesconto'
      'Pec_ValorProduto=Pec_ValorProduto'
      'CompradorNome=CompradorNome'
      'FornecedorNome=FornecedorNome')
    DataSet = Pedido
  end
  inherited DS: TDataSource
    DataSet = Pedido
  end
  inherited Q: TFDQuery
    IndexesActive = False
    IndexFieldNames = 'pci_pedidocompra'
    SQL.Strings = (
      
        'select pci_pedidocompra,pci_produto,pro_descricao,pci_precounita' +
        'rio, pci_quantidade, pci_valortotal from pedidocompraitem'
      
        'inner join produto on produto.pro_codigo = pedidocompraitem.pci_' +
        'produto')
    object Qpci_pedidocompra: TIntegerField
      FieldName = 'pci_pedidocompra'
      Origin = 'pci_pedidocompra'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object Qpci_produto: TIntegerField
      FieldName = 'pci_produto'
      Origin = 'pci_produto'
      Required = True
    end
    object Qpro_descricao: TStringField
      FieldName = 'pro_descricao'
      Origin = 'pro_descricao'
      Required = True
      Size = 50
    end
    object Qpci_precounitario: TBCDField
      FieldName = 'pci_precounitario'
      Origin = 'pci_precounitario'
      Precision = 14
      Size = 2
    end
    object Qpci_quantidade: TBCDField
      FieldName = 'pci_quantidade'
      Origin = 'pci_quantidade'
      Precision = 14
      Size = 2
    end
    object Qpci_valortotal: TBCDField
      FieldName = 'pci_valortotal'
      Origin = 'pci_valortotal'
      Precision = 14
      Size = 2
    end
  end
  object DBITEM: TfrxDBDataset
    UserName = 'ITEM'
    CloseDataSource = False
    FieldAliases.Strings = (
      'pci_pedidocompra=pci_pedidocompra'
      'pci_produto=pci_produto'
      'pro_descricao=pro_descricao'
      'pci_precounitario=pci_precounitario'
      'pci_quantidade=pci_quantidade'
      'pci_valortotal=pci_valortotal')
    DataSet = Item
    BCDToCurrency = False
    Left = 416
    Top = 152
  end
  object Pedido: TFDQuery
    Active = True
    IndexesActive = False
    IndexFieldNames = 'pec_codigo'
    Connection = D.FDCon
    SQL.Strings = (
      
        'select pec_codigo,pec_data,Pec_Baixa,Pec_Fornecedor,Pec_Comprado' +
        'r, iif(Pec_Status = '#39'P'#39','#39'Pendente'#39','#39'Baixado'#39') as Status,Pec_Valo' +
        'rTotal,Pec_ValorDesconto,Pec_ValorProduto,'
      
        'Pco.Pes_nome as CompradorNome, Pfo.Pes_Nome as FornecedorNome fr' +
        'om pedidocompra pc'
      'inner join pessoa Pco on Pco.Pes_codigo = pc.Pec_Comprador'
      'inner join pessoa Pfo on Pfo.Pes_codigo = pc.Pec_Fornecedor')
    Left = 80
    Top = 60
    object Pedidopec_codigo: TFDAutoIncField
      FieldName = 'pec_codigo'
      Origin = 'pec_codigo'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object Pedidopec_data: TDateField
      FieldName = 'pec_data'
      Origin = 'pec_data'
      Required = True
    end
    object PedidoPec_Baixa: TDateField
      FieldName = 'Pec_Baixa'
      Origin = 'Pec_Baixa'
    end
    object PedidoPec_Fornecedor: TIntegerField
      FieldName = 'Pec_Fornecedor'
      Origin = 'Pec_Fornecedor'
      Required = True
    end
    object PedidoPec_Comprador: TIntegerField
      FieldName = 'Pec_Comprador'
      Origin = 'Pec_Comprador'
    end
    object PedidoStatus: TStringField
      FieldName = 'Status'
      Origin = 'Status'
      ReadOnly = True
      Required = True
      Size = 8
    end
    object PedidoPec_ValorTotal: TBCDField
      FieldName = 'Pec_ValorTotal'
      Origin = 'Pec_ValorTotal'
      Precision = 14
      Size = 2
    end
    object PedidoPec_ValorDesconto: TBCDField
      FieldName = 'Pec_ValorDesconto'
      Origin = 'Pec_ValorDesconto'
      Precision = 14
      Size = 2
    end
    object PedidoPec_ValorProduto: TBCDField
      FieldName = 'Pec_ValorProduto'
      Origin = 'Pec_ValorProduto'
      Precision = 14
      Size = 2
    end
    object PedidoCompradorNome: TStringField
      FieldName = 'CompradorNome'
      Origin = 'CompradorNome'
      Size = 50
    end
    object PedidoFornecedorNome: TStringField
      FieldName = 'FornecedorNome'
      Origin = 'FornecedorNome'
      Size = 50
    end
  end
  object Item: TFDQuery
    Active = True
    IndexFieldNames = 'pci_pedidocompra'
    MasterSource = DS
    MasterFields = 'pec_codigo'
    Connection = D.FDCon
    SQL.Strings = (
      
        'select pci_pedidocompra,pci_produto,pro_descricao,pci_precounita' +
        'rio, pci_quantidade, pci_valortotal from pedidocompraitem'
      
        'inner join produto on produto.pro_codigo = pedidocompraitem.pci_' +
        'produto')
    Left = 112
    Top = 60
    object Itempci_pedidocompra: TIntegerField
      FieldName = 'pci_pedidocompra'
      Origin = 'pci_pedidocompra'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object Itempci_produto: TIntegerField
      FieldName = 'pci_produto'
      Origin = 'pci_produto'
      Required = True
    end
    object Itempro_descricao: TStringField
      FieldName = 'pro_descricao'
      Origin = 'pro_descricao'
      Required = True
      Size = 50
    end
    object Itempci_precounitario: TBCDField
      FieldName = 'pci_precounitario'
      Origin = 'pci_precounitario'
      Precision = 14
      Size = 2
    end
    object Itempci_quantidade: TBCDField
      FieldName = 'pci_quantidade'
      Origin = 'pci_quantidade'
      Precision = 14
      Size = 2
    end
    object Itempci_valortotal: TBCDField
      FieldName = 'pci_valortotal'
      Origin = 'pci_valortotal'
      Precision = 14
      Size = 2
    end
  end
end
